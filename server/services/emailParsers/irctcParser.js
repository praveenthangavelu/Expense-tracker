...
export const parseIRCTC = ({ body, subject, date }) => {
  const result = { source: 'IRCTC', type: 'expense', category: 'Transport' };
  result.subCategory = 'Train';
  ...
  // Extract amount
  const amountMatch = body.match(/(?:total fare|ticket fare|amount paid|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:total fare|ticket fare|amount paid|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  // Extract reference
  const refMatch = body.match(/(?:pnr\s*no\.?\s*|pnr\s*#?\s*|transaction\s*id\s*[:\-]?\s*)([0-9]+)/i);
  
  result.amount = amount;
  result.merchant = 'IRCTC';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'IRCTC train ticket';
