...
export const parseNykaa = ({ body, subject, date }) => {
  const result = { source: 'Nykaa', type: 'expense', category: 'Shopping' };
  result.subCategory = 'Personal Care';
  ...
  // Extract amount
  const amountMatch = body.match(/(?:grand total|total|paid)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:grand total|total|paid)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  result.merchant = 'Nykaa';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Nykaa order';
