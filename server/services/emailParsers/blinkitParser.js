...
export const parseBlinkit = ({ body, subject, date }) => {
  const result = { source: 'Blinkit', type: 'expense', category: 'Food' };
  ...
  // Extract amount
  const amountMatch = body.match(/(?:grand total|bill total|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:grand total|bill total|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  result.merchant = 'Blinkit';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Blinkit groceries';
