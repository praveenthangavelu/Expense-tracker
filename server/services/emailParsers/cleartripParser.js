...
export const parseCleartrip = ({ body, subject, date }) => {
  const result = { source: 'Cleartrip', type: 'expense', category: 'Transport' };
  ...
  // Extract reference
  const refMatch = body.match(/(?:trip\s*id\s*[:\-]?\s*)([a-zA-Z0-9\-]+)/i);
  
  result.amount = amount;
  result.merchant = 'Cleartrip';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Cleartrip booking';
