...
export const parseAjio = ({ body, subject, date }) => {
  const result = { source: 'Ajio', type: 'expense', category: 'Shopping' };
  ...
  // Extract amount
  const amountMatch = body.match(/(?:grand total|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:grand total|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
