...
export const parseBESCOM = ({ body, subject, date }) => {
  const result = { source: 'BESCOM', type: 'expense', category: 'Bills' };
  ...
  // Extract amount
  const amountMatch = body.match(/(?:bill amount|amount paid|total|amount)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:bill amount|amount paid|total|amount)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  result.merchant = 'BESCOM';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'BESCOM Electricity bill';
