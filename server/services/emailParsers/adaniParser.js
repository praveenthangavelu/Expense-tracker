...
export const parseAdaniElectricity = ({ body, subject, date }) => {
  const result = { source: 'Adani Electricity', type: 'expense', category: 'Bills' };
  result.subCategory = 'Electricity';
  ...
  // Extract amount
  const amountMatch = body.match(/(?:bill amount|amount paid|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:bill amount|amount paid|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  // Extract reference
  const refMatch = body.match(/(?:consumer\s*no\.?\s*)([0-9]+)/i);
  
  result.amount = amount;
  result.merchant = 'Adani Electricity';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Adani Electricity bill';
