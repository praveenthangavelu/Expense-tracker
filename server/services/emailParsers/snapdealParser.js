...
export const parseSnapdeal = ({ body, subject, date }) => {
  const result = { source: 'Snapdeal', type: 'expense', category: 'Shopping' };
  ...
  // Extract amount
  const amountMatch = body.match(/(?:total amount|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:total amount|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  result.merchant = 'Snapdeal';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Snapdeal purchase';
