...
export const parseZomato = ({ body, subject, date }) => {
  const result = { source: 'Zomato', type: 'expense', category: 'Food' };
  result.subCategory = 'Online Food Delivery';
  ...
  // Extract amount
  const amountMatch = body.match(/(?:total paid|bill total|grand total|total|amount)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:total paid|bill total|grand total|total|amount)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
