...
export const parseOla = ({ body, subject, date }) => {
  const result = { source: 'Ola', type: 'expense', category: 'Transport' };
  ...
  // Extract amount
  const amountMatch = body.match(/(?:fare|total|amount)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:fare|total|amount)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  // Extract reference
  const refMatch = body.match(/(?:booking\s*id\s*[:\-]?\s*|crn\s*#?\s*)([0-9]+)/i);
  
  result.amount = amount;
  result.merchant = 'Ola';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Ola ride';
