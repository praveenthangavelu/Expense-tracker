...
export const parseMyntra = ({ body, subject, date }) => {
  const result = { source: 'Myntra', type: 'expense', category: 'Shopping' };
  ...
  // Extract amount
  const amountMatch = body.match(/(?:order total|total paid|amount paid|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:order total|total paid|amount paid|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  result.merchant = 'Myntra';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Myntra order';
