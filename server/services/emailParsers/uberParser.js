...
export const parseUber = ({ body, subject, date }) => {
  const result = { source: 'Uber', type: 'expense', category: 'Transport' };
  ...
  // Extract amount
  const amountMatch = body.match(/(?:total|fare|amount paid)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:total|fare|amount paid)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  // Extract reference
  const refMatch = body.match(/(?:trip\s*id\s*[:\-]?\s*|receipt\s*#?\s*)([a-zA-Z0-9\-]+)/i);
  
  result.amount = amount;
  result.merchant = 'Uber';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Uber ride';
