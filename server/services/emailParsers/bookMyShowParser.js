...
export const parseBookMyShow = ({ body, subject, date }) => {
  const result = { source: 'BookMyShow', type: 'expense', category: 'Entertainment' };
  ...
  // Extract amount
  const amountMatch = body.match(/(?:amount paid|total|paid)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:amount paid|total|paid)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  // Extract reference
  const refMatch = body.match(/(?:booking\s*id\s*[:\-]?\s*|booking\s*#?\s*)([a-zA-Z0-9\-]+)/i);
  
  result.amount = amount;
  result.merchant = 'BookMyShow';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Movie Ticket';
