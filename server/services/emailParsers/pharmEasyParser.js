...
export const parsePharmEasy = ({ body, subject, date }) => {
  const result = { source: 'PharmEasy', type: 'expense', category: 'Health' };
  result.subCategory = 'Medicines';
  ...
  // Extract amount
  const amountMatch = body.match(/(?:bill total|grand total|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:bill total|grand total|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  // Extract reference
  const refMatch = body.match(/(?:order\s*id\s*[:\-]?\s*)([a-zA-Z0-9\-]+)/i);
  
  result.amount = amount;
  result.merchant = 'PharmEasy';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'PharmEasy medicines';
