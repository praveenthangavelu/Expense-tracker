...
export const parseSpotify = ({ body, subject, date }) => {
  const result = { source: 'Spotify', type: 'expense', category: 'Entertainment' };
  ...
  // Extract reference
  const refMatch = body.match(/(?:receipt\s*no\.?\s*)([0-9]+)/i);
  
  result.amount = amount;
  result.merchant = 'Spotify';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Spotify Premium';
