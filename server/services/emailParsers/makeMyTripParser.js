...
export const parseMakeMyTrip = ({ body, subject, date }) => {
  const result = { source: 'MakeMyTrip', type: 'expense', category: 'Transport' };
  ...
  // Extract amount
  const amountMatch = body.match(/(?:total paid|grand total|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:total paid|grand total|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  // Extract reference
  const refMatch = body.match(/(?:booking\s*id\s*[:\-]?\s*|booking\s*reference\s*#?\s*)([a-zA-Z0-9\-]+)/i);
  
  result.amount = amount;
  result.merchant = 'MakeMyTrip';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'MMT booking';
