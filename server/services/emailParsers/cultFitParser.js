...
export const parseCultFit = ({ body, subject, date }) => {
  const result = { source: 'Cult Fit', type: 'expense', category: 'Health' };
  result.subCategory = 'Fitness';
  ...
  // Extract amount
  const amountMatch = body.match(/(?:amount paid|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:amount paid|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  result.merchant = 'Cult Fit';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Cult.fit membership';
