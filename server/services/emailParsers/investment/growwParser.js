...
export const parseGroww = ({ body, subject, date }) => {
  const result = { source: 'Groww', type: 'expense', category: 'Other' };
  ...
  // Extract amount
  const amountMatch = body.match(/(?:amount|total|invested)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:amount|total|invested)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  result.merchant = 'Groww';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'Investment via Groww';
