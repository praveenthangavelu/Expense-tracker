...
export const parseRedBus = ({ body, subject, date }) => {
  const result = { source: 'RedBus', type: 'expense', category: 'Transport' };
  result.subCategory = 'Bus';
  ...
  // Extract amount
  const amountMatch = body.match(/(?:total fare|amount paid|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i) || subject.match(/(?:total fare|amount paid|total)\s*[:\-]?\s*(?:₹|rs\.?|inr)\s*([\d,]+\.?\d*)/i);
  ...
  // Extract reference
  const refMatch = body.match(/(?:ticket\s*no\.?\s*|booking\s*id\s*[:\-]?\s*)([a-zA-Z0-9\-]+)/i);
  
  result.amount = amount;
  result.merchant = 'RedBus';
  result.date = date;
  result.referenceId = refMatch ? refMatch[1] : null;
  result.note = 'redBus ticket';
